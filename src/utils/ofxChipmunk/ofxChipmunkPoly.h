...
class ofxChipmunkPoly : public ofxChipmunkBaseShape{
	...
	public:
		...
		void kill() {
			...
			if(space != NULL && polyShapes.size()){
			
				for(int i = 0; i < polyShapes.size(); i++){
					cpSpaceRemoveShape(space, polyShapes[i]);
					cpShapeFree(polyShapes[i]);
					//printf("freeing %i \n", i);
				}
				polyShapes.clear();
		...
		//-----------------------------------------------------------------------------------------
		virtual void makePolyShape(float x, float y, float shapeMass, float elasticity, float fricton, bool bShapeStatic = false){
		...
		virtual void setCollisionID(int id){
			if(polyShapes.size() ){
				for(int i = 0; i < polyShapes.size(); i++){
					polyShapes[i]->collision_type = id;
				}
			}	
		}
		...
		//-----------------------------------------------------------------------------------------
		virtual void makePolyShape(float x, float y, float shapeMass, bool bShapeStatic = false){
