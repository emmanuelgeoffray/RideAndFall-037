...
//-----------------------------------------------------------------------------------------
void fishBirdManager::setup(ofPoint pos, int numFishBirds, string birdTexPath, string fishTexPath){
	...
	for(int i = 0; i < fb.size(); i++){
		...
		float angle = ofMap(i, 0, fb.size(), 0, TWO_PI);
		
		fb[i].birthAngle = angle;
		
		float x = pos.x + cos(angle) * ofRandom(120, 160);
		float y = pos.y + sin(angle) * ofRandom(120, 160);
		float z = pos.z - 10;
...
//-----------------------------------------------------------------------------------------
vector <baseElement *> fishBirdManager::getElements(){
...
//-----------------------------------------------------------------------------------------
void fishBirdManager::doBurst(){
	bNeedToBurst = true;
}

//-----------------------------------------------------------------------------------------
void fishBirdManager::update(){
	...
	for(int i = 0; i < fb.size(); i++){
		...
		float driftAmnt = ofMap(ofSignedNoise(0, ofGetElapsedTimef() * 0.07), 1, -1, 100, 400, true);			
		...
		if( fb[i].state == BF_WAITING_FIRST && bNeedToBurst ){
